...
def downsample_voxel_grid(points: np.ndarray, targets: np.ndarray, voxel_size=0.001) -> tuple:
    ...
    for idx, voxel_idx in enumerate(voxel_indices):
        ...
        if voxel_key not in voxel_dict:
            voxel_dict[voxel_key] = {"points": [], "targets": []}
        voxel_dict[voxel_key]["points"].append(points[idx])
        voxel_dict[voxel_key]["targets"].append(targets[idx])
    sampled_points = []
    sampled_targets = []
    for voxel_key, voxel_data in voxel_dict.items():
        voxel_points = np.array(voxel_data["points"])
        voxel_targets = np.array(voxel_data["targets"])
        centroid = voxel_points.mean(axis=0)
        majority_target = np.bincount(voxel_targets.astype(int)).argmax()
...
def downsample_feature_based(points: np.ndarray, targets: np.ndarray, npoints: int = 8000, k=20, high_variance_threshold=0.05, low_variance_rate=0.1) -> tuple:
    ...
    print(f"The shape of the point cloud before feature based downsampling: {points.shape}")
...
def downsample_test(points: np.ndarray, targets: np.ndarray, npoints=2000, voxel_size=0.01, plane_threshold=0.02, higher_downsample_rate=0.9) -> tuple:
